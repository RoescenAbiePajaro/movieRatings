...
<main class="flex flex-col flex-1 p-4 bg-black text-white min-h-screen">
    <section class="py-8">
        <h2 class="text-center text-4xl font-bold mb-10">Movie Ratings</h2>
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
            {#each movies as movie}
                <Link to={`/movie/${movie.id}`} class="group relative cursor-pointer overflow-hidden rounded-lg shadow-lg">
                    <img
                        src={movie.image}
                        alt={movie.title}
